...
# ══════════════════════════════════════════════════════════════════════════════
#  PaddleOCR Engine — Multi-lang cache (loaded once per language)
# ══════════════════════════════════════════════════════════════════════════════
_engines = {}
...
def _get_engine(lang: str = "latin"):
    if lang not in _engines:
        ...
        _engines[lang] = PaddleOCR(
            use_angle_cls=False,  # ✅ يوفر ~150MB — كافٍ لـ latin/english
            use_gpu=False,
            lang=lang,
            show_log=False,
            enable_mkldnn=False   # ✅ يقلل memory footprint على Linux
        )
    return _engines[lang]

# ✅ تحميل مسبق عند بدء السيرفر (أفضل من التحميل عند أول طلب)
_get_engine("latin")
...
def extract_layout_from_image(image_path: str, lang: str = "latin") -> dict:
    try:
        ...
        result = engine.ocr(image_path, cls=False)  # ✅ cls=False لأن use_angle_cls=False
        blocks = _parse_ocr_result(result)
        full_text = "\n".join(b["text"] for b in blocks if b["confidence"] > 0.5)
        del result
        gc.collect()
        return {"text": full_text, "blocks": blocks, "image_width": img_w, "image_height": img_h}
...
def extract_layout_from_pil(pil_image: Image.Image, lang: str = "latin") -> dict:
    try:
        ...
        result       = engine.ocr(img_array, cls=False)  # ✅ cls=False
        blocks       = _parse_ocr_result(result)
        full_text    = "\n".join(b["text"] for b in blocks if b["confidence"] > 0.5)
